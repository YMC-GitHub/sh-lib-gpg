...
BACKUP_DES_PATH="secret/gpg"

# del files in  "$GPG_KEY_PATH"
#gpg_backup_file_del "$GPG_KEY_PATH" "$BACKUP_DES_PATH"
# add files to "$GPG_KEY_PATH" from "$BACKUP_DES_PATH"
mkdir -p "$GPG_KEY_PATH" ; gpg_backup_file_gen "$GPG_KEY_PATH" "$BACKUP_DES_PATH"
# use files in "$GPG_KEY_PATH"
gpg_pri_key_import "$GPG_PRI_KEY_FILE"
gpg_pub_key_import "$GPG_PUB_KEY_FILE"
#gpg_rev_key_import "$GPG_REV_KEY_FILE"
...
# file usage
# sample/tom/ap.bak.use.sh
# sample/tom/ap.bak.use.sh "yemiancheng" "hualei03042013@163.com" "20-09-20" "$GPG_PASSPHRASE" "1y"
# sample/tom/ap.bak.use.sh "yemiancheng" "ymc.github@gmail.com" "19-06-30" "yemiancheng521" "2y"
# sample/tom/ap.bak.use.sh "$GPG_USER_NAME" "$GPG_USER_EMAIL" "$GPG_USER_NOTE" "$GPG_PASSPHRASE" "$GPG_KEY_EXP"